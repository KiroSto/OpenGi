...
describe("Successfully executing the entire workflow test", function () {
  ...
  it("executing the entire workflow with all the mandatory fields", function () {
    ...
    cy.get(":nth-child(3) > .css-2smw7z-SectionItems").should("have.text","No bespoke business lines created");
    cy.get(":nth-child(6) > .css-2smw7z-SectionItems").should("have.text","No delegated authority products created");
    navigation.getAccountsButton().click();
    cy.get(".css-1lpj7mv-BankAccounts").should("have.text","No other client money bank accounts created");
  });
});
